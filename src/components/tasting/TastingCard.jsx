...
export default function TastingCard({ tasting, onDelete }) {
  ...
  return (
    <div className="tasting-card">
      ...
      {/* Cabecera */}
      <div className="tasting-card-header" onClick={() => setExpanded(!expanded)}>
        <div className="tasting-card-info">
          <h3 className="tasting-card-name">
            {cafe?.nombre || 'Café sin nombre'}
          </h3>
          <div className="tasting-card-meta">
            {cafe?.origen && <span><FaEarthAfrica /> {cafe.origen}</span>}
            {cafe?.proceso && <span><FaGear /> {cafe.proceso}</span>}
            {cafe?.tueste && <span><FaFire /> {cafe.tueste}</span>}
          </div>
        </div>
