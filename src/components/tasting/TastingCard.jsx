...
function TastingChips({ cafe, tasting }) {
  const chips = [
    cafe?.origen && {
      key: "origen",
      icon: CHIP_ICONS.origen,
      label: `Origen: ${cafe.origen}`,
    },
    cafe?.proceso && {
      key: "proceso",
      icon: CHIP_ICONS.proceso,
      label: `Proceso: ${cafe.proceso}`,
    },
    cafe?.tueste && {
      key: "tueste",
      icon: CHIP_ICONS.tueste,
      label: `Tueste: ${cafe.tueste}`,
    },
    cafe?.finca && {
      key: "finca",
      icon: CHIP_ICONS.finca,
      label: `Finca: ${cafe.finca}`,
    },
    cafe?.sca && {
      key: "sca",
      icon: CHIP_ICONS.sca,
    ...
    cafe?.marca && {
      key: "marca",
      icon: CHIP_ICONS.marca,
      label: `Marca: ${cafe.marca}`,
    },
    tasting.precio && {
      key: "precio",
      icon: CHIP_ICONS.precio,
      label: `Precio: ${tasting.precio}`,
    },
  ].filter(Boolean);
    ...
    <div className="tasting-card">
      ...
      {expanded && (
        <div className="tasting-card-detail">
          ...
          {/* Link de compra y botón compartir */}
          {(tasting.link_compra || true) && (
            <div className="tasting-card-purchase">
              ...
              <button
                className="tasting-card-share"
                onClick={() => share("tasting-beanlog")}
                disabled={sharing}
                title="Compartir cata"
              >
                {sharing ? <FaShareSquare /> : <><FaShareSquare /> Compartir</>}
              </button>
            </div>
          )}
