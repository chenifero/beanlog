...
export default function PostCard({ post, onDelete }) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const comment = await postService.addComment(user.id, post.id, newComment)
    ...
    <article className="post-card">
      ...
      <div className="post-header">
        ...
        <div className="post-author-info">
          <span className="post-username">{post.profiles?.username || 'Usuario'}</span>
          <span className="post-level">{levelLabel(post.profiles?.experience_level)}</span>
        </div>
        <div className="post-meta">
          <span className="post-time">{timeAgo(post.created_at)}</span>
          {/* Solo el autor ve el botón de eliminar */}
          {isOwner && (
            <button className="post-delete-btn" onClick={handleDelete} title="Eliminar">✕</button>
          )}
        </div>
      </div>
      ...
      {post.type === 'tasting' && post.tastings && (
        <div className="post-tasting-info">
          <span className="post-tasting-name">
            <BiSolidCoffeeBean /> {post.tastings.cafes_master?.nombre || 'Café sin nombre'}
          </span>
          {post.tastings.cafes_master?.origen && (
            <span className="post-tasting-origin">{post.tastings.cafes_master.origen}</span>
          )}
          {post.tastings.puntuacion && (
            <span className="post-tasting-score">{post.tastings.puntuacion}/10</span>
          )}
        </div>
      )}
      ...
      {post.type === 'visit' && post.coffee_shops && (
        <div className="post-visit-info">
          <span className="post-visit-name"><FaLocationDot /> {post.coffee_shops.nombre}</span>
          {post.rating && <span className="post-tasting-score">{post.rating}/10</span>}
        </div>
      )}
      ...
      <div className="post-actions">
        <button
          ...
          onClick={handleLike}
        >
          {liked ? <FcLike /> : <FcLikePlaceholder />} {likesCount > 0 && likesCount}
        </button>
        ...
        <div className="post-comments">
          ...
          {comments.map(comment => (
            <div key={comment.id} className="post-comment">
              <span className="post-comment-user">{comment.profiles?.username}</span>
          ...
          <div className="post-comment-input-row">
            ...
            />
            <button className="post-comment-send" onClick={handleAddComment}>→</button>
          </div>
        </div>
      )}
