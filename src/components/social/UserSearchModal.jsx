    ...
    <div className="usearch-overlay" onClick={onClose}>
      <div className="usearch-modal" onClick={e => e.stopPropagation()}>
        <div className="usearch-header">
          ...
          <button className="usearch-close" onClick={onClose}>✕</button>
        </div>
        <input
        ...
        <div className="usearch-results">
          ...
          {results.map(profile => (
            <div key={profile.id} className="usearch-item">
              <div className="usearch-avatar" onClick={() => goToProfile(profile.username)}>
                {profile.avatar_url
                  ? <img src={profile.avatar_url} alt={profile.username} />
                  : profile.username?.charAt(0).toUpperCase()
                }
              </div>
              <div className="usearch-info" onClick={() => goToProfile(profile.username)}>
                <p className="usearch-username">{profile.username}</p>
                {profile.bio && <p className="usearch-bio">{profile.bio}</p>}
              </div>
              <button
